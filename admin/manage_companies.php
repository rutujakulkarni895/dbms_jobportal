...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $name = $_POST['name'];
    $email = $_POST['email'];
    $contact = $_POST['phone']; // fixed variable name
    ...
    if (isset($_POST['company_id']) && $_POST['company_id'] !== '') {
        ...
        // Add - insert current timestamp
        $stmt = $conn->prepare("INSERT INTO companies (name, email, phone, address, created_at) VALUES (?, ?, ?, ?, NOW())");
...
<div class="container mt-5">
    ...
    <form method="POST" class="card p-4 mt-4 shadow-sm">
        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-3">
                <label>Contact</label>
                <input type="text" name="contact" class="form-control" value="<?= htmlspecialchars($edit_data['phone'] ?? '') ?>">
            </div>
            <div class="col-md-6 mb-3">
                <label>Address</label>
                <input type="text" name="address" class="form-control" value="<?= htmlspecialchars($edit_data['address'] ?? '') ?>">
            </div>
        </div>
            ...
            <?php while ($row = $companies->fetch_assoc()): ?>
                <tr>
                    ...
                    <td>
                        ...
                        <a href="?delete=<?= $row['id'] ?>" class="btn btn-sm btn-danger delete-btn">Delete</a>
                    </td>
                </tr>
            <?php endwhile; ?>
        </tbody>
